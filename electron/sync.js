import { apiClient, getApiConfig, saveApiConfig } from './api_config.js';

let database;

export async function initSyncService(dbInstance) {
  database = dbInstance;
  
  // Start sync loop
  console.log('🚀 Sync Service Initialized. Loop: 30s');
  setInterval(syncLoop, 30000); // Every 30 seconds
...
async function downloadUpdates() {
  try {
    ...
    try {
      ...
      if (userData) {
        ...
        config.loggedInUser = {
          ...
          permissions: userData.roles?.[0]?.permissions?.map((p) => p.name) || [],
    ...
    } catch (e) { console.error('Failed to sync locations:', e.message); }
    ...
    } catch (e) { console.error('Failed to sync tax rates:', e.message); }
    ...
    } catch (e) { console.error('Failed to sync products and lots:', e.message); }
    ...
    } catch (e) { console.error('Failed to sync customers:', e.message); }
